...
// Define navigation callback type
type NavigateCallback = (view: 'dashboard' | 'excalidraw' | 'reactflow' | 'broadband' | 'database' | 'logs' | 'template') => void;
...
export const GlobalSearch = ({ onNavigate }: { onNavigate?: NavigateCallback }) => {
      ...
      if (searchQuery.length > 1) {
        ...
        try {
            ...
            const staticPages: SearchResultItem[] = [
                ...
                { title: 'Tickets', to: '/tickets', type: 'page', id: 'p2' },
                { title: 'Customers', to: '/customers', type: 'page', id: 'p3' },
                { title: 'Topology', to: '/topology', type: 'page', id: 'p4' },
                { title: 'Database', to: '/database', type: 'page', id: 'p5' },
                { title: 'Monitor', to: '/monitor', type: 'page', id: 'p6' },
                { title: 'Settings', to: '/settings', type: 'page', id: 'p7' }
            ];
  ...
  const handleSelect = (item: SearchResultItem) => {
    ...
    if (item.type === 'page' && item.to && onNavigate) {
      // Map URL paths to view states
      const pathToViewMap: Record<string, Parameters<NavigateCallback>[0]> = {
        '/': 'dashboard',
        '/database': 'database',
        '/logs': 'logs',
        '/template': 'template'
      ...
      const view = pathToViewMap[item.to];
      if (view) {
        onNavigate(view);
      }
